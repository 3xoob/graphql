...
class GraphQLService {
    ...
    async getUserInfo() {
        const [data, error] = await this.#fetchData(queries_1.GET_USER_INFO);
        if (error !== null) {
            return [null, error];
        }
    ...
    async getTopTransaction() {
        const [data, error] = await this.#fetchData(queries_1.GET_TOP_TRANSACTION);
        if (error !== null) {
            return [null, error];
        }
    ...
    async getAuditStatus() {
        const [data, error] = await this.#fetchData(queries_1.GET_AUDIT_STATUS);
        if (error !== null) {
            return [null, error];
        }
        if ('user' in data && Array.isArray(data.user)) {
    ...
    async getTotalXP() {
        const [data, error] = await this.#fetchData(queries_1.GET_TOTAL_XP);
        if (error !== null) {
            return [null, error];
        }
    ...
    async getUserTechnologies() {
        const [data, error] = await this.#fetchData(queries_1.GET_USER_Technologies);
        if (error !== null) {
            return [null, error];
        }
    ...
    async getUserTechnicalSkills() {
        const [data, error] = await this.#fetchData(queries_1.GET_USER_Technical_Skills);
        if (error !== null) {
            return [null, error];
        }
        if ('transaction' in data) {
    ...
    async getAuditRatio() {
        const [data, error] = await this.#fetchData(queries_1.GET_AUDIT_RATIO);
        if (error !== null) {
            return [null, error];
        }
        if ('user' in data && Array.isArray(data.user)) {
